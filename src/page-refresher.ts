export const pageRefresher = (): void => {
  ...
  form.addEventListener("submit", (event: Event) => {
    ...
    const [url, interval] = getFormData(event.target as HTMLFormElement);
  ...
  const getFormData = (target: HTMLFormElement): [string, number] => {
  ...
  const isUrlValid = (maybeUrl: string) => {
    try {
